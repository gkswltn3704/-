...
public class MysqlDAO implements DAO {
	...
	public List<DeptDTO> selectAll() {
		...
		RowMapper<DeptDTO> rm = new RowMapper<DeptDTO>() {
			...
			public DeptDTO mapRow(ResultSet rs, int rowNum) throws SQLException {
				/*
				 * DeptDTO dto = new DeptDTO(); int deptno = rs.getInt(1); String dname =
				 * rs.getString(2); String loc = rs.getString(3);
				 * 
	...
	public void insertOne(DeptDTO dto) {
		...
		// jdbcTemplate.update(sql문, ?에 들어갈 값을순서대로 입력)
		int result = jdbcTemplate.update(sb.toString(), dto.getDeptno(),
							dto.getDname(), dto.getLoc());		//내가 데이터를 get으로 가져와서 써야 자동으로 set이 된다.
